...
{
    shmdt(ptr_.get());
    if(shmctl(shm_id_, IPC_RMID, nullptr)){
        printf("共享内存删除失败\n");
    }
}
...
// void ShmCommon::writeCostmapShm()
// {
//     //** boost::unique_lock<Costmap2D::mutex_t> lock(*(costmap_->getMutex()));
//     unsigned char *data = costmapRy_->getCharMap();
//     int size = costmapRy_->getCellSizeX() * costmapRy_->getCellSizeY();
//     if (Semaphore_P(sem_id_) == 1)
//     {
//         shm_data_->size_x = costmapRy_->getSizeX();
//         shm_data_->size_y = costmapRy_->getSizeY();
//         shm_data_->origin_x = costmapRy_->getOriginX();
//         shm_data_->origin_y = costmapRy_->getOriginY();
//         shm_data_->resolution = costmapRy_->getResolution();
//         for (size_t i = 0; i < size; i++)
//         {
//             shm_data_->costmap[i] = data[i];
//         }
//         Semaphore_V(sem_id_);
//     }
//     else
//     {
//         LOG(INFO) << ("write shm failed");
//     }
// }